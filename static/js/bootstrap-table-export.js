    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showExport) {
            ...
            if (!$export.length) {
                ...
                if (that.options.enableExportSelection && that.$toolbar){
                    var selectionClass, ref, $toolbar, $selection;
                    selectionClass = "bs-bars pull-" + that.options.toolbarAlign;
                    var $toolbar_div = $("<div name='o_general_toolbar' class='"+ selectionClass +"'></div>")

                    $toolbar = (ref = that.options.toolbar)? ref : (function(){that.$toolbar.append($toolbar_div);return $toolbar_div;})();
                    $toolbar = $toolbar instanceof jQuery ? $toolbar : $($toolbar);

                    var stateField = that.header.stateField;

                    $selection = $("<select class='form-control'>"+
                        "<option value='basic'>导出当前页</option>" +
                        "<option value='all'>导出所有</option> " +
                        (stateField?"<option value='selected'>导出选中记录</option>":"")+
                        "</select>");
                    $selection.val(that.options.exportDataType);
                    $toolbar.append($selection);
                    $selection.on('change',function(){
                        that.options.exportDataType = $selection.val();
                    });
                }
                    ...
                    if (that.options.exportDataType === 'all' && that.options.pagination) {
                        if(that.options.sidePagination === "server"){
                           var options = that.options;
                            var cur_pageSize = options.pageSize
                            var cur_pageNumber = options.pageNumber;
                            var ajaxOptions = options.ajaxOptions

                            options.pageSize = 99999999;
                            ...
                            options.pageSize = cur_pageSize;
                            options.pageNumber = cur_pageNumber;
                            options.ajaxOptions = ajaxOptions;
                            that.refresh(options);
                            if (stateField) {
                                that.showColumn(stateField);
                            }
                        }else if(that.options.sidePagination === "client"){
